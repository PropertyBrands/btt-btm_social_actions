...
BTM.social_actions = {
    ...
    attachTracking: function() {
        BTM.social_actions.bindFBevents();
        BTM.social_actions.bindTwitterEvents();
    },
    ...
    extractParamFromTweetUri: function(uri, paramName) {
       if (!uri) {
		return;
	  }
    ...
    bindTwitterEvents: function() {
	  ...
	  if (event) {
	    ...
	    if (event.target && event.target.nodeName == 'IFRAME') {
		  targetUrl = BTM.social_actions.extractParamFromTweetUri(event.target.src, 'url');
	    }
		alert('event fired!');
...
//Instantiating _gaq queue
        BTM.social_actions.attachTracking();
...
/**
 * Load libraries for external services
 */
